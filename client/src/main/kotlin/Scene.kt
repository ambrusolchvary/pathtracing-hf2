...
class Scene (
  ...
  /*  crypto.getRandomValues(randomInts)
    ...
    var k = 0;
    ii@for(i in 0 until 128){
      for(j in 0 until 1024){
        rndv4[i].set(
          randomInts[k*4+0].toFloat() / Int.MAX_VALUE.toFloat() * 0.5f,
          randomInts[k*4+1].toFloat() / Int.MAX_VALUE.toFloat() * 0.5f,
          randomInts[k*4+2].toFloat() / Int.MAX_VALUE.toFloat() * 0.5f,
          randomInts[k*4+3].toFloat() / Int.MAX_VALUE.toFloat() * 0.5f
        )
        k = k + 1;
        if(rndv4[i].xyz.length() < 1.0f){
          break@ii
        }
      }
    }
*/  
    ...
    traceProgram.draw(this, camera, *lights.toTypedArray(), *quadrics.toTypedArray())
